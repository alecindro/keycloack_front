...
export class LoginService {
  ...
  login(): void {
    ...
    //location.href = `${location.origin}${this.location.prepareExternalUrl('api/users')}`;
    alert('location: ' + `${location.origin}${this.location.prepareExternalUrl('oauth2/authorization/oidc')}`);

    // location.href = `${location.origin}${this.location.prepareExternalUrl('api/users/authorities')}`;
  }
